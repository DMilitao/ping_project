...
TEST_F(NSocketTest, CanSendMessage){
    ...
    EXPECT_NE(expect_socket_.Send(message_), -1);
...
TEST_F(NSocketTest, CanCreateAddress){
    ...
    EXPECT_TRUE(expect_socket_.SetIP(port,ip));

    EXPECT_TRUE(expect_socket_.Close());
}

TEST_F(NSocketTest, CanStartListen){
    EXPECT_TRUE(expect_socket_.CreateSocket());

    EXPECT_TRUE(expect_socket_.StartListen(10));

    EXPECT_TRUE(expect_socket_.Close());
}

TEST_F(NSocketTest, CanAcceptConnection){
    uint16_t port = 8080;
    uint32_t ip = 0;
    ...
    std::unique_ptr<NSocket> new_socket = std::make_unique<NSocket>();
    new_socket = expect_socket_.AcceptConnection();

    EXPECT_TRUE(new_socket != nullptr);
    EXPECT_TRUE(expect_socket_.Close());
}

TEST_F(NSocketTest, CanConnectToServer){
    uint16_t port = 8080;
    uint32_t ip = 0;

    EXPECT_TRUE(expect_socket_.CreateSocket());

    EXPECT_TRUE(expect_socket_.NewConnection(port, ip));
